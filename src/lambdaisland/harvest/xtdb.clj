(ns lambdaisland.harvest.xtdb
  "XTDB support for factories"
  (:require [xtdb.api :as xt]
            [lambdaisland.harvest.kernel :as hk]))

(defn result-tx
  "Turn a Harvest result map into a XTDB transaction."
  [{:harvest.result/keys [value linked]}]
  (into []
        (keep #(when (:xt/id %)
                 [::xt/put %]))
...
(defn after-build-factory-impl
  "`:harvest.hooks/after-build-factory` implementation which assigns a `:xt/id` and
...
(defn build
  "Like [[lambdaisland.harvest/build]] but returns data that is suitable for
  ...
  ([template {:keys [xt-id-fn]
   ...
   (let [opts (update
     ...
     (hk/build {:xt-id-fn xt-id-fn} template opts))))
